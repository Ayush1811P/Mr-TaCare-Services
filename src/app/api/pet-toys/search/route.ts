...
import { normalizeSerpApiProduct, normalizeSerperProduct } from '@/services/pet-toys/normalizeProduct';
import { rankProducts } from '@/services/pet-toys/rankProducts';
...
async function fetchFromSerpApi(queries: string[], animal: PetToysAnimalType, lifeStage: PetToysLifeStage): Promise<PetToysProduct[]> {
  if (serpApiQuotaExhausted) {
    console.log('[PetToys] SerpApi quota previously exhausted -> skipping');
    return [];
  }

  const allProducts: PetToysProduct[] = [];
  const uniqueUrls = new Set<string>();
  
  console.log('[PetToys] Provider: SerpApi');
    ...
    try {
      ...
      if (res.status === 400 || res.status === 403 || res.status === 429 || res.status === 402) {
        ...
        if (
          ...
          console.log('[PetToys] SerpApi quota exhausted → switching to Serper');
          return allProducts;
        }
      }
      ...
      console.error(`[PetToys] SerpApi query failed or timed out: ${q}`);
    }
  }
  return allProducts;
}

async function fetchFromSerper(queries: string[], animal: PetToysAnimalType, lifeStage: PetToysLifeStage): Promise<PetToysProduct[]> {
  if (serperQuotaExhausted) {
    console.log('[PetToys] Serper quota previously exhausted -> skipping');
    return [];
  }
  ...
  console.log('[PetToys] Provider: Serper');
  ...
  for (const q of queries) {
    ...
    while (currentSerperKeyIndex < SERPER_API_KEYS.length) {
      try {
        ...
        if (res.status === 400 || res.status === 403 || res.status === 429 || res.status === 402) {
          console.log(`[PetToys] Serper key at index ${currentSerperKeyIndex} failed (Status: ${res.status})`);
        ...
        console.error(`[PetToys] Serper query failed or timed out: ${q}`);
...
export async function GET(request: Request) {
  ...
  const animal = searchParams.get('animal') as PetToysAnimalType;
  const breed = searchParams.get('breed');
  const lifeStage = searchParams.get('lifeStage') as PetToysLifeStage;
  ...
  if (breed) {
    queries.push(`${breed} ${lifeStage} ${animal} toys`);
    queries.push(`${breed} ${animal} toys`);
  }
  queries.push(`${lifeStage} ${animal} toys`);
  queries.push(`${animal} toys`);
  ...
  try {
    ...
    if (cached && !error) {
      ...
      if (age < ttl) {
        console.log(`[PetToys] Serving from cache: ${cacheKey}`);
        ...
        console.log(`[PetToys] Cache expired for: ${cacheKey}`);
      }
    }
  } catch (err) {
    console.error('[PetToys] Cache read error:', err);
  }

  let allProducts: PetToysProduct[] = [];
  ...
  try {
    ...
    console.log('[PetToys] All Serper keys exhausted or failed → switching to SerpApi');
  }
  ...
  if (allProducts.length === 0) {
    console.log('[PetToys] Both providers unavailable');
  ...
  try {
    if (ranked.length > 0) {
      await supabaseServer.from('pet_toys_cache').upsert({
        cache_key: cacheKey,
        animal,
        breed,
      ...
      console.log(`[PetToys] Saved to cache: ${cacheKey}`);
    }
  } catch (err) {
    console.error('[PetToys] Cache write error:', err);
  }
