...
async function populate() {
  console.log("Starting Pet Toys cache population...");
  ...
  for (const [animal, animalBreeds] of Object.entries(breeds)) {
    ...
    for (const stage of stages) {
      console.log(`Fetching generic ${animal} toys - ${stage}`);
      try {
        const url = `http://localhost:3000/api/pet-toys/search?animal=${animal}&lifeStage=${stage}`;
    ...
    for (const breed of animalBreeds) {
      for (const stage of stages) {
        console.log(`Fetching ${animal} toys - ${breed} - ${stage}`);
        try {
          const url = `http://localhost:3000/api/pet-toys/search?animal=${animal}&breed=${breed}&lifeStage=${stage}`;
          const res = await fetch(url);
          console.log(` -> Status: ${res.status}`);
  ...
  console.log("Pet Toys cache population complete.");
}
